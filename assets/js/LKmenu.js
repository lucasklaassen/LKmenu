                ...
                if (!isFirefox && !isIE8()) {
                    (function ($) {
                        $("html").on('touchstart click', function (e) {
                            ...
                            if ($(e.target).hasClass(settings.menuIcon) && !$menu.hasClass('menu-visible')) {
                                animate('rightAnimate');
                            } else if ((!$(e.target).parents(settings.mainMenu).length && $menu.hasClass('menu-visible') && !$(e.target).hasClass(settings.mainMenu)) || $(e.target).hasClass(settings.menuIcon)) {
                                animate('leftAnimate');
                            } else {
                                return true;
                            }
                            $menu.on(transitionEnd, function () {
                                $menu.removeClass('animating leftAnimate rightAnimate').toggleClass('menu-visible');
